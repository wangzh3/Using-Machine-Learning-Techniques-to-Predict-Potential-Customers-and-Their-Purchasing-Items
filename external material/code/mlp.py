...
shuffledata1=np.loadtxt(root+"yoochoose-data/yoochoose-clickssave1.txt",dtype=np.float)
...
for epoch in range(EP):
    ...
    if(epoch%10==0):
        torch.save(net, root+"mlp/net" + str(epoch) + ".pkl")

torch.save(net, root+"mlp/net.pkl")
file.close()
#train end

#test start
now = time.time()
net2=torch.load(root+"mlp/net0.pkl")
loss_func=torch.nn.CrossEntropyLoss()
...
A=np.loadtxt(root+"log.txt",dtype=np.float)
epoch=A[:,0]
loss=A[:,1]
acc=A[:,2]
plt.figure("loss")
